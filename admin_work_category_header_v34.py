...
class AdminWorkCategoryHeaderV34:
    ...
    This wrapper intentionally does not depend on PATH_INFO or query-string
    routing.  It runs outside Flask so it can return JSON before any legacy
    admin/task POST validator or global error handler can turn the request into
    a 404 HTML page.
    """
    ...
    def _health(start_response):
        body = b"admin_work_category_header=v34 active=1"
        start_response(
            "200 OK",
            [
    ...
    def __call__(self, environ, start_response):
        if (environ.get("PATH_INFO") or "") == "/__health/admin-work-category-v34":
