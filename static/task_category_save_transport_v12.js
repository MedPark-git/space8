(() => {
  ...
  const ENDPOINT = "/tasks/category-manager/v12";
  ...
  const submitCategoryForm = (action, values) => {
    ...
    appendHidden(form, "category_action", action);
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", reopenManager);
  else reopenManager();
})();
