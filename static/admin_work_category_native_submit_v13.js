(() => {
  ...
  const currentAdminAction = () => {
    // Post back to the exact page that is already open and working instead of
    // assuming that the Flask admin route is literally /admin.
  ...
  const submitNative = (operation, payload = {}) => {
    ...
    form.method = 'post';
    form.action = currentAdminAction();
  ...
  window.__MEDPARK_ADMIN_WORK_CATEGORY_TRANSPORT__ = 'v13-native-current-url';
})();
