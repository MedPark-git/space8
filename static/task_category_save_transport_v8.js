(() => {
  ...
  const prepareNativePost = (form, action, values) => {
    ...
    hidden(form, "_category_transport", "v8");
  ...
  document.addEventListener("submit", (event) => {
    ...
    if (operation === "middle_add") {
      prepareNativePost(form, "add", { department_id: deptId, middle_name: newName, small_name: "" });
    } else if (operation === "small_add") {
      prepareNativePost(form, "add", { department_id: deptId, middle_name: middle, small_name: newName });
    } else if (operation === "middle_rename") {
      prepareNativePost(form, "rename_middle", { department_id: deptId, old_middle_name: middle, new_middle_name: newName });
    } else if (operation === "small_rename") {
      prepareNativePost(form, "rename_small", { work_category_id: smallId, new_small_name: newName });
    } else {
