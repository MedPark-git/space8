...
FLASK_APP = core.app
TRANSPORT_VERSION = "task-category-transport-v5"
...
def _allowed_department(department_id):
    ...
    if not _is_admin() and current_user.department_id != department.id:
        return None, _json_error("본인 소속 부서(팀)의 중분류·소분류만 관리할 수 있습니다.", 403, "CATEGORY_FORBIDDEN")
    return department, None


def _visible_categories():
    rows = core.active_work_categories() if _is_admin() else core.active_work_categories([current_user.department_id])
...
def _handle_add():
    ...
    if len(middle_name) > 100 or len(small_name) > 150:
        return _json_error("업무구분 이름 길이를 확인해 주세요.", 400, "CATEGORY_NAME_TOO_LONG")
    category = core.db.session.scalar(select(core.WorkCategory).where(
    ...
    if category and category.active:
        return _json_ok("이미 등록된 업무구분입니다. 저장된 기초자료를 다시 불러왔습니다.", category=_category_payload(category), categories=_visible_categories(), duplicate=True)
    created = category is None
    if category:
        category.active = True
    else:
        category = core.WorkCategory(department_id=department.id, middle_name=middle_name, small_name=small_name, active=True)
        core.db.session.add(category)
    try:
        core.db.session.flush()
        core.audit("TASK_WORK_CATEGORY_CREATE" if created else "TASK_WORK_CATEGORY_REACTIVATE", f"work-category:{category.id}", {
        ...
        core.db.session.rollback()
        return _json_error(f"업무구분 등록 중 오류가 발생했습니다. ({type(exc).__name__})", 500, "CATEGORY_ADD_ERROR")
    return _json_ok("업무구분을 등록했습니다.", category=_category_payload(category), categories=_visible_categories())
...
def _handle_rename_middle():
    ...
    rows = core.db.session.scalars(select(core.WorkCategory).where(
        core.WorkCategory.department_id == department.id,
    ...
    conflict = core.db.session.scalar(select(core.WorkCategory.id).where(
        core.WorkCategory.department_id == department.id,
    ...
    try:
        ...
        core.audit("TASK_WORK_CATEGORY_MIDDLE_RENAME", f"department:{department.id}", {
        ...
        core.db.session.rollback()
        return _json_error(f"중분류 수정 중 오류가 발생했습니다. ({type(exc).__name__})", 500, "MIDDLE_RENAME_ERROR")
...
def _handle_rename_small():
    ...
    try:
        ...
        core.audit("TASK_WORK_CATEGORY_SMALL_RENAME", f"work-category:{category.id}", {
        ...
        core.db.session.rollback()
        return _json_error(f"소분류 수정 중 오류가 발생했습니다. ({type(exc).__name__})", 500, "SMALL_RENAME_ERROR")
...
@FLASK_APP.get("/__health/task-category-transport-v5")
def task_category_transport_health():
    return jsonify({"ok": True, "transport": TRANSPORT_VERSION, "path": "/tasks/new", "actions": sorted(HANDLERS)})
...
class TaskCategoryTransportMiddleware:
    ...
    def __call__(self, environ, start_response):
        ...
        action = str((query.get("category_action") or [""])[0]).strip()
        transport = str((query.get("_category_transport") or [""])[0]).strip()
        if path == "/tasks/new" and method == "POST" and action in ALLOWED_ACTIONS and transport == "v5":
            with self.flask_app.request_context(environ):
                if not current_user.is_authenticated:
                    response = self.flask_app.make_response(_json_error("로그인이 필요합니다.", 401, "LOGIN_REQUIRED"))
                else:
                    core.csrf.protect()
