...
def _handle_rename_middle():
    ...
    if not old_name or not new_name:
        return _json_error("기존 중분류명과 새 중분류명을 확인해 주세요.", 400, "MIDDLE_REQUIRED")
    if len(new_name) > 100:
        return _json_error("중분류명은 100자 이하로 입력해 주세요.", 400, "MIDDLE_TOO_LONG")
...
def _task_category_action():
    if request.path != "/tasks/new" or request.method != "POST":
        return None
    ...
    # This handler is inserted before Flask-WTF's normal CSRF before_request hook,
    # so explicitly protect these same-path management requests.
    core.csrf.protect()

    if action == "add":
        return _handle_add()
    if action == "rename_middle":
        return _handle_rename_middle()
    if action == "rename_small":
        return _handle_rename_small()
    if action == "list":
        return jsonify({"ok": True, "categories": _visible_categories()})

    return _json_error("지원하지 않는 업무구분 작업입니다.", 400, "UNKNOWN_CATEGORY_ACTION")


app.before_request_funcs.setdefault(None, []).insert(0, _task_category_action)
